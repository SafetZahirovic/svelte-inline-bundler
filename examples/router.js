...
router.get("/", headerMiddleware, async (_req, res) => {
    ...
    res.send(`<head>${ssr.head}<style>${ssr.css.code}</style></head><body>${ssr.html}</body><script>${dom}</script>`);
});
router.get("/api", headerMiddleware, (_req, res) => {
    res.send("HELLO IT IS WORKING");
});
router.get("/ssr/:view", headerMiddleware, async (req, res) => {
    ...
    const { module, props } = viewsAndProps.find(({ view }) => view === requestedView);
    const bundle = await generateBundle({
        props,
        module,
        name: "App",
...
router.get("/dom/:view", headerMiddleware, async (req, res) => {
    const requestedView = req.params["view"];
    if (!viewsAndProps.find(({ view }) => view === requestedView)) {
        res.send("No views were found for " + requestedView).status(404);
    }
...
router.get("/hydratable/:view", headerMiddleware, async (req, res) => {
    const requestedView = req.params["view"];
    if (!viewsAndProps.find(({ view }) => view === requestedView)) {
        res.send("No views were found for " + requestedView).status(404);
    }
